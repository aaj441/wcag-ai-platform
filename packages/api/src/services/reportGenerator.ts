...
import { Violation, EmailDraft, LegacyViolation } from '../types';
import { applyTemplateSubstitution } from './keywordExtractor';
...
/**
 * Generate email body with keyword substitution
 * Supports dynamic placeholders like {{violation_count}}, {{wcag_criterion}}, etc.
 */
export function generateEmailWithTemplateSubstitution(
  draft: EmailDraft
): string {
  if (!draft.violations || draft.violations.length === 0) {
    return draft.body;
  }
  
  return applyTemplateSubstitution(draft.body, draft.violations);
}

/**
 * Generate personalized email draft with keywords
 */
export function generatePersonalizedDraft(
  recipientName: string,
  company: string,
  violations: LegacyViolation[],
  template?: string
): string {
  const defaultTemplate = `Dear ${recipientName},

I hope this email finds you well. I'm reaching out regarding a recent WCAG accessibility audit of the ${company} website.

Our automated scanning tool has identified {{violation_count}} accessibility violations that could impact user experience and legal compliance. ${violations.some(v => v.severity === 'critical') ? 'The most critical issues require immediate attention.' : ''}
...
The violations affect WCAG criteria: {{wcag_criteria_list}}

I've attached a detailed report with specific recommendations and step-by-step remediation guidance.

Would you be available for a brief call this week to discuss the findings?

Best regards,
WCAG AI Platform Team`;

  const bodyTemplate = template || defaultTemplate;
  return applyTemplateSubstitution(bodyTemplate, violations);
}
