...
Provide your response in the following JSON format:
{
  ...
  private getMockFix(request: AIFixRequest): AIFixResponse {
    ...
    if (mockFixes[request.issueType]) {
      return mockFixes[request.issueType];
    }

    // Try partial match (for similar issue types)
    const key = Object.keys(mockFixes).find((k) =>
      request.issueType.toLowerCase() === k.toLowerCase() ||
      request.issueType.toLowerCase().includes(k.toLowerCase())
    );

    return mockFixes[key!] || {
